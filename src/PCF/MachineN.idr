...
data Stack : Ty -> Ty -> Type where
  ...
  IS : Term g a -> Term (N::g) a -> Env g -> Stack a c -> Stack N c
...
step (St (Lam t)                    e       (CS c s) r) = Just $ St  t                  (c::e)              s    r
step (St (App t u)                  e             s  r) = Just $ St  t                      e  (Cl u e `CS` s)   r
step (St (Fix t)                    e             s  r) = Just $ St  t       (Cl (Fix t) e::e)              s    r
step (St (If0 p t f)                e             s  r) = Just $ St  p                      e     (IS t f e s)   r
step (St  Zero                      e  (IS t _ e1 s) r) = Just $ St  t                      e1              s    r
step (St (Succ n)                   e  (IS _ f e1 s) r) = Just $ St  f             (Cl n e::e1)             s    r
step (St (Succ n)                   e             s  r) = Just $ St  n                      e               s (S r)
step  _                                               = Nothing  

runMach : Term [] a -> (Nat, Maybe (State a))
runMach t = iterCount step $ St t [] NS Z
